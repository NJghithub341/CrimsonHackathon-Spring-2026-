import { Question, QUESTIONS, getRandomQuestions, getQuestionById, getDifficultyPoints } from '../data/questions';
...
export class QuestionService {
   ...
   */
  async generateBattleQuestionsLegacy(): Promise<Question[]> {
  ...
  async generateQuestion(
    ...
    if (this.geminiService.isReady()) {
      return await this.geminiService.generateQuestion(
        language,
        difficulty,
        category || 'algorithms'
      );
    } else {
      // Fallback to existing questions if Gemini is not available
      const availableQuestions = this.getQuestions({
      ...
      throw new Error('No questions available and AI service not configured');
    }
  }

  async generateHint(questionId: string, userAnswer?: number, timeSpent?: number): Promise<string> {
    const question = this.getQuestion(questionId);
    if (!question) {
    ...
    if (this.geminiService.isReady()) {
      return await this.geminiService.generateHint(question, userAnswer, timeSpent);
    } else {
      return 'Think about the fundamental concepts and try to eliminate obviously incorrect options.';
    }
  }
  ...
  async analyzeUserCode(
    ...
    if (this.geminiService.isReady()) {
      return await this.geminiService.analyzeCode(code, language, context);
    } else {
      return {
        issues: [],
        suggestions: ['AI code analysis not available'],
        complexity: 'unknown',
        readability: 5
      };
    }
  ...
  async generateAdaptiveQuestions(
    ...
    if (this.geminiService.isReady()) {
      return await this.geminiService.generatePracticeSet(
        userWeakAreas,
        language,
        difficulty,
        count
      ...
      return this.getPracticeQuestions(
        userWeakAreas as Question['category'][],
